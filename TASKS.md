...
- [x] DX-014: PMO workload-hook og Vitest-dækning
  - Formål: Flytte den tunge `useMemo`-logik i `PmoPage` ud i en dedikeret hook, så komponenten holder sig til rendering og beregningerne kan testes/genbruges.
  - Ændringer:
    1) Ny `usePmoWorkload` (inkl. helper `buildPmoWorkload`) der filtrerer aktive projekter, konverterer uge-nøgler til datoer og grupperer timer pr. medarbejder og lokation.
    2) Vitest i `src/hooks/usePmoWorkload.test.ts` der dækker dato-filtrering, ugyldige uge-nøgler og at inaktive projekter ignoreres.
    3) `PmoPage` bruger hooken i stedet for inlined logik, hvilket reducerer komponentstørrelse uden at ændre UI.
  - Test (TDD):
    1) `npm run test`
  - Accept: PMO-overblikket viser samme data som før, hook-testene kører og logikken er klar til genbrug andre steder i PMO-modulet.
